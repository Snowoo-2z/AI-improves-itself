...
| **Recherche web** | `colab/` | L'IA programme des tâches (`search`/`deep`/`fetch`) ; le notebook Colab v2 les exécute (multi-moteurs sans clé, lecture auto des pages, résumé local optionnel par clé API, synchro robuste : réveil Render, statuts monotones, push idempotent) ; les résultats reviennent au site, qui les **structure puis vérifie** (2 appels IA) avant de les inscrire dans la base de connaissances. |
