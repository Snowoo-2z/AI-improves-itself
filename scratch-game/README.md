...
> Miniatures à la taille exacte de la scène Scratch (**480 × 360**) : `miniature.png` (menu) et
> `miniature-combat.png` (combat) — prêtes à servir de vignette de projet.
...
- **Logo dessiné** : « ARENA CLASH » est un vrai lettrage vectoriel (contours de la police, dégradé
  or, écusson rose/violet, coins lumineux) affiché en sprite au-dessus du menu, pas un simple texte tamponné.
- **Menu** complet : Jouer / Boutique / Commandes, aperçu de ton combattant, pièces, arme équipée et progression.
...
| `tools/thumb.js` | assemble des captures 480×360 en une planche (`node thumb.js ../preview.png 960 out/a.svg out/b.svg ...`) |
| `tools/svg2png.js` | convertit un SVG en PNG (`node svg2png.js in.svg out.png 640`) |
| `tools/` | harnais de test headless : `cd tools && npm i && bash test.sh` rejoue tous les scénarios dans la vraie VM Scratch (dégâts par arme, niveaux, boutique, sauvegarde, combat complet) et capture les écrans dans `tools/out/` ; `node play.js ../dist/ArenaClash.sb3 t_miniatures.json` puis `node thumb.js` régénèrent `miniature*.png` (480×360) et `preview*.png` (960×720) |
